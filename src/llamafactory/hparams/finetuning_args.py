...
class QApolloArguments:
    r"""Arguments pertaining to the Quantized APOLLO (Q-APOLLO) algorithm."""
    ...
    qapollo_target: str = field(
        ...
        metadata={
            "help": (
                "Name(s) of modules to apply Q-APOLLO. Use commas to separate multiple modules. "
                "Use `all` to specify all the linear modules."
            )
        },
    )
    qapollo_rank: int = field(
        default=16,
        metadata={"help": "The rank of Q-APOLLO gradients."},
    )
    qapollo_update_interval: int = field(
        default=200,
        metadata={"help": "Number of steps to update the Q-APOLLO projection."},
    )
    qapollo_scale: float = field(
        default=32.0,
        metadata={"help": "Q-APOLLO scaling coefficient."},
    )
    qapollo_proj: Literal["svd", "random"] = field(
        default="random",
        metadata={"help": "Type of Q-APOLLO low-rank projection algorithm (svd or random)."},
    )
    qapollo_proj_type: Literal["std", "right", "left"] = field(
        default="std",
        metadata={"help": "Type of Q-APOLLO projection."},
    )
    qapollo_scale_type: Literal["channel", "tensor"] = field(
        default="channel",
        metadata={"help": "Type of Q-APOLLO scaling (channel or tensor)."},
    )
